...
Every RFC that adds a browser capability touches the same seven places, in this order:
...
4b. **`internal/mcp/bind.go`** — the matching `boundBrowser` forwarder, so an MCP request's cancellation reaches the new method too.
   `TestBoundBrowserBindsEveryMethod` fails on a method it inherits from the embedded `Browser` (reported as `<autogenerated>`) instead of overriding — the same shape of guard as step 4's `TestDispatchCoversBrowser`, and just as easy to skip by accident.
