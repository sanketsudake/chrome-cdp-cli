...
4. **Save, then confirm the write actually happened — never assume from the click alone:**
...
5. **Branch on the exit code** (see `drive-chrome-cdp`): `0` means confirmed; anything else means the save is unproven — do not tell the user it worked.
...
- Never click the save/submit control until the read-back in step 3 matches what you meant to write.
